...
const submitHandler = e => {
  ...
  while (mensagem.match(regexFone)) {
    ...
    link.href =
      ...
      mensagem.match(regexFone)[0].replace(/[\(\)\s\-]/g, '')
...
socket.on('broadMessage', mensagem => {
  ...
  while (msgRecebida.innerHTML.match(regexNegrito)) {
    let negrito = msgRecebida.innerHTML
      .replace('*', ' <b>')
      .replace('*', '</b> ')
